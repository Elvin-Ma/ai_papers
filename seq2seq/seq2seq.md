...
![table1](images/seq2seq-table1.jpg)
...
![table2](images/seq2seq-table2.jpg)

## 3.7 长句子的表现
![figure3](images/seq2seq-figure3.jpg)

令我们惊讶的是，LSTM在长句子上表现出色，这在图3中有定量的展示。表3呈现了几个长句子及其翻译的示例。<br>

![table3](images/seq2seq-table3.jpg)

## 3.8 模型分析

![figure2](images/seq2seq-figure2.jpg)
*图2：该图显示了在处理图中短语后获得的LSTM隐藏状态的二维PCA投影。这些短语是根据含义进行聚类的，在这些示例中，主要是通过单词的顺序来确定含义，这在词袋模型中很难捕捉到。请注意，两个聚类具有相似的内部结构。* <br>

我们模型的一个吸引人的特点是它能够将一系列单词转化为一个**固定维度**的向量。图2可视化了一些学习到的表示。该图清楚地显示，这些表示对单词的**顺序敏感**，但对于使用被动语态替代主动语态而言相对不敏感。通过使用PCA进行了二维投影。<br>

# 4 相关工作
在神经网络应用于机器翻译方面有大量的研究工作。迄今为止，将循环神经网络语言模型（RNNLM）[23]或前馈神经网络语言模型（NNLM）[3]应用于机器翻译任务最简单且最有效的方法是对强大的机器翻译基线[22]的nbest列表进行重新评分，这可靠地提高了翻译质量。<br>

最近，研究人员开始探索将源语言信息纳入NNLM的方法。这方面的工作包括Auli等人[1]将NNLM与输入句子的主题模型相结合，改进了重新评分的性能。Devlin等人[8]采用了类似的方法，但他们将NNLM整合到机器翻译系统的解码器中，并使用解码器的对齐信息为NNLM提供输入句子中最有用的单词。他们的方法非常成功，相对于基线系统取得了很大的改进。<br>

我们的工作与Kalchbrenner和Blunsom [18]密切相关，他们首次将输入**句子映射到向量**，然后再映射回句子，尽管他们使用卷积神经网络将句子映射到向量，但会丢失单词的顺序。与本文类似，Cho等人[5]使用了类似LSTM的循环神经网络架构将句子映射到向量，然后再映射回句子，尽管他们的主要重点是将他们的神经网络集成到SMT(统计机器翻译)系统中。Bahdanau等人[2]也尝试使用神经网络进行直接翻译，他们使用了**注意力机制**来解决Cho等人[5]在长句子上性能较差的问题，并取得了令人鼓舞的结果。同样，Pouget-Abadie等人[26]尝试通过以产生平滑翻译为目标的方式翻译源句子的片段，类似于基于短语的方法，以解决Cho等人[5]的记忆问题。我们怀疑他们只需简单地将网络训练于反转的源句子上，就可以实现类似的改进。<br>

Hermann等人[12]的研究也专注于端到端训练，他们的模型通过前馈网络将输入和输出表示为向量，并将它们映射到空间中的相似点。然而，他们的方法不能直接生成翻译：为了获得翻译，他们需要在预先计算的句子数据库中查找最接近的向量，或对句子进行重新评分。<br>

# 5 结论
在本研究中，我们展示了一个具有有限词汇表且几乎不对问题结构做出假设的大型深度LSTM，可以在大规模机器翻译任务上胜过词汇表无限制的标准SMT系统。我们简单的基于LSTM的方法在机器翻译任务上的成功表明，在有足够的训练数据的情况下，它在许多其他序列学习问题上也会表现出色。<br>

我们对通过**反转源句子中的单词**所获得的改进程度感到惊讶。我们得出结论，找到一个具有**最多短期依赖关系**的问题编码是非常重要的，因为这使得学习问题变得更简单。特别是，尽管我们无法训练非反转翻译问题上的标准RNN（如图1所示），我们相信当源句子被反转时，标准RNN应该很容易进行训练（尽管我们没有进行实验证实）。<br>

这些发现表明，深度LSTM在处理**序列学习**问题上具有潜力，并且通过适当的问题编码和数据训练，可以取得显著的性能改进。未来的工作可以探索其他序列学习任务，并进一步优化和改进基于LSTM的方法。我们对LSTM能够准确翻译非常长的句子感到惊讶。最初，我们确信LSTM在长句子上会失败，因为其有限的记忆容量，而其他研究者也报告了与我们类似的模型在长句子上表现不佳的情况[5, 2, 26]。然而，对反转数据集进行训练的LSTM在翻译长句子时几乎没有困难。<br>

最重要的是，我们证明了一种简单、直接且相对未经优化的方法可以胜过SMT系统，因此进一步的工作很可能会导致更高的翻译准确性。这些结果表明，我们的方法很可能在其他具有挑战性的序列到序列问题上表现出色。<br>
