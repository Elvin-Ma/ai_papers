...
## 5.6 迈向万亿参数模型
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;结合专家并行性、模型并行性和数据并行性，我们设计了两个大型的Switch Transformer模型，分别拥有3950亿和1.6万亿个参数。我们研究了这些模型在上游预训练作为语言模型和下游微调性能方面的表现。这两个不同模型的参数、每个序列的FLOPS和超参数列在下表9中。其中包括Transformer的标准超参数，如dmodel、df f、dkv、头数和层数，以及一个较不常见的特征F F NGEGLU，它指的是FFN层的一种变体，其中扩展矩阵被两组权重替代，并进行非线性组合（Shazeer，2020）。<br>
