...
其中$\mathcal{U}=\left\lbrace u_{1}, \ldots, u_{n}\right\rbrace$, 是tokens的文本向量，n是网络层数，$W_{e}$是token嵌入矩阵（embedding matrix），$W_{p}$是位置嵌入矩阵.
其中$\mathcal{U}=\left\lbrace u_{1}, \ldots, u_{n}\right\rbrace$ , W_{p}
