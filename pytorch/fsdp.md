...
- 如何在不塑造任何张量存储的情况下创建模型实例，将初始化**推迟(postponing)到**将具体设备上的存储器附加到张量上之后。<br>
- 如何确保模型参数的准确初始化与用户的实现保持一致，即使模型太大无法适应单个GPU。 <br>
...
![figure7](images/fsdp-figure7.jpg)

![figure8](images/fsdp-figure8.jpg)
...
### 7.1.1 管道并行性
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;通过使用FSDP将每个独立的管道阶段包装起来，可以将管道并行性与FSDP进行功能集成。然而，由于**管道并行性将输入的小批量划分为更小的微批量**，FSDP中默认的完全分片策略需要**针对每个微批量取消分片模型参数**。因此，将这些方法与默认的FSDP配置结合使用可能会导致显著的通信开销。幸运的是，FSDP提供了替代的分片策略，在前向传递后可以保持参数未分片，避免每个微批量进行不必要的AllGather通信。值得承认的是，这要求在GPU设备上存储整个管道阶段的参数，但FSDP仍然可以减少内存使用，因为**它仍然对梯度和优化器状态进行分片**。<br>

### 7.1.2 张量并行计算
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;与FSDP相比，张量并行计算在计算过程中保持参数(parameter)分片，这在任何子模块太大无法适应GPU内存时是必要的。目前，PyTorch提供了一个名为parallelize_module的**原型功能**，可以与FSDP结合使用以实现二维并行计算。它通过**将设备组织成一个二维网格来工作**，其中PyTorch的分布式张量DTensor在一个维度上管理张量并行计算，而FSDP在另一个维度上应用参数分片的数据并行计算。**这两个维度分别用于通信激活和参数**。通常我们将张量并行计算的通信限制在节点内部，以利用更高的网络带宽，并允许FSDP的通信在节点之间的另一个网格维度上进行操作。<br>

## 7.2 限制
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;在我们与生产和研究应用程序的工作中，我们遇到了一些与FSDP相关的限制。本节旨在讨论两个棘手的注意事项，这些注意事项并不容易被察觉，并在故障排除时带来重大挑战。<br>

### 7.2.1 数学等效性。
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;FSDP不能保证始终实现与本地训练相同的数学等效性，特别是在优化器计算方面。这是因为**优化器步骤是在分片参数上进行的**，其数据布局是FSDP的FlatParameter分片算法的函数，该算法不尊重各个参数的边界。因此，任何依赖于**原始**参数的未分片值（例如向量范数）、张量结构（例如近似二阶优化器）或需要全局状态的优化器计算都将变得无效。解决这个问题需要不均匀的分片、填充或额外的通信，而这些都会对性能造成损害。如何在分片的同时进行这种优化器计算的共同设计是一个开放的研究问题。<br>

### 7.2.2 共享参数
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;对于共享参数，FSDP必须确保不将它们展开为多个FlatParameter，并确保在所有使用情况下适当地对其进行**未分片处理**。如果处理不正确，PyTorch可能会引发有关缺少张量存储或大小不匹配的错误，这可能发生在先前的FSDP单元对已经被前面的FSDP单元重新分片的共享参数进行使用时。目前的建议是构建FSDP单元，使共享参数属于最低公共祖先单元，以**确保共享参数在所有使用情况下都是未分片的**。这可能需要对模型结构进行一些检查，以确保正确处理，但可能会导致在较长的时间间隔内不希望的保持FlatParameter未分片，因此我们正在研究改进共享参数处理的方法。<br>

# 8 结论
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;本文阐明了PyTorch 2.0版本中FullyShardedDataParallel的基本原理、设计理念和实现方法。FSDP通过一系列先进技术实现了易用性和高效性，包括延迟初始化、灵活的分片策略、通信重叠和预取，以及限制通信集合的速率。所有这些技术都与其他关键的PyTorch组件密切协同设计，以确保解决方案的可靠性和稳健性。评估结果表明，FSDP能够实现几乎线性可扩展的大型语言和推荐模型的训练。<br>
