...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;为了回答这些问题, 通过公式 $U^{T}WV^{T}$ 将W投影到∆W的r维子空间上，其中U/V 是∆W的左/右奇异向量矩阵。然后，我们比较 $||U^{T}WV^{T}||$ 和 $||W_{F}||$ 之间的Frobenius范数。作为对比，我们还通过将U、V替换为W的前r个奇异向量或一个随机矩阵，计算 $||U^{\top} W V^{\top}||$ 。<br>

![table7](images/lora-table7.jpg)

*(表7：U_{T}W_{q}V_{T} 的Frobenius范数，其中U和V分别是(1) ∆Wq，(2) Wq或(3)随机矩阵的左/右前r个奇异向量方向的奇异矩阵。权重矩阵取自GPT-3的第48层。)*

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;从表7中我们得出几个结论。首先，与随机矩阵相比，∆W与W具有更强的相关性，这表明∆W放大了已经存在于W中的一些特征。其次，∆W并不重复W的顶部奇异方向，而是**只放大在W中没有强调的方向**。第三，放大因子相当巨大：对于r = 4，21.5 ≈ 6.91/0.32。有关为什么r = 64具有较小放大因子的原因，请参见第H.4节。我们还在第H.3节提供了一种可视化方法，展示了随着从Wq中包含更多顶部奇异方向，相关性的变化情况。这表明**低秩自适应矩阵可能放大了在一般预训练模型中学习但没有强调的特定下游任务的重要特征**。<br>

# 8 结论和未来工作
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;在硬件要求和为不同任务托管独立实例的存储/切换成本方面，微调庞大的语言模型代价高昂。我们提出了一种高效的自适应策略LoRA，它既不引入推理延迟，也不降低输入序列长度，同时保持高模型质量。重要的是，它允许在部署为服务时快速切换任务，通过共享绝大部分模型参数。虽然我们专注于Transformer语言模型，但所提出的原则通常适用于具有密集层的任何神经网络。<br>

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;未来工作可以从以下几个方向展开：<br>
1. 可以将LoRA与其他高效的自适应方法相结合，可能提供正交的改进。
2. 微调或LoRA背后的机制还远未清楚——在预训练期间学习的特征如何转化为在下游任务上表现良好？我们认为相比完全微调，LoRA使得回答这个问题更加可行。
3. 我们大多数情况下依靠启发式方法来选择应用LoRA的权重矩阵。是否有更加原则性的方法来处理这个问题？
