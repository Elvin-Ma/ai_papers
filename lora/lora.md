...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;为了回答这些问题，通过公式 $U^{T}WV^{T} 将W投影到∆W的r维子空间上，其中U/V 是∆W的左/右奇异向量矩阵。然后，我们比较 
 $|| U^{T} W V^{T} ||_{F}$ 和 $\|W\|_{F}$ 之间的Frobenius范数。作为对比，我们还通过将U、V替换为W的前r个奇异向量或一个随机矩阵，计算 $\left\|U^{\top} W V^{\top}\right\|_{F}$ 。<br>
