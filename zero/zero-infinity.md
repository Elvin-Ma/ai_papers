...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**残差状态的内存**：残差状态主要包括激活内存，其大小取决于模型架构、批次大小 (𝑏𝑠𝑧) 和序列长度 (𝑠𝑒𝑞)，可能会非常大。从积极的一面来看，可以通过激活检查点 [29] 显著减少激活所需的内存，但这会以**0.33倍**的额外计算为代价，换取激活内存的减少。例如，Turing-NLG 17.2B和GPT-3 175B等大型模型都是使用**激活检查点**进行训练的。存储激活检查点所需的内存估计为：<br>
